...
class Test_PasswordRecovery(unittest.TestCase):
  ...
  def test_password_recovery_reset(self):
      ...
      Functions.GUIdisplay.testName = "Password Recovery"
      ...
      if check != "You will receive an email with instructions about how to reset your password in a few minutes.":
        automatedSmokeTest.GUIFunctions.outputDisplayConsole("The confirmation message saying 'You will receive an email with instructions about how to reset your password in a few minutes.' is not displayed correctly" , Functions.GUIdisplay.testName, 's')
      time.sleep(2)
        ...
        if Test_PasswordRecovery.is_element_present(self, By.XPATH, "//div[@id='password']/div[@class='LXRPh']/div[@class='dEOOab RxsGPe']"):
          ...
          if "reset_password_token" in driver.current_url:
            ...
            if Test_PasswordRecovery.is_element_present(self, By.XPATH, "//div[@class ='hidden-print']/div[@id='alertMsgContainer']/div[@class='alert alert-notice']"):
              automatedSmokeTest.GUIFunctions.outputDisplayConsole("Password was succesfully resetted." , Functions.GUIdisplay.testName, 's')
              ...
              driver.quit()
              automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: Password needed to be contain certain characters. Please re-write the password to change." , Functions.GUIdisplay.testName, 'ie')
            else:
              driver.quit()
              automatedSmokeTest.GUIFunctions.outputDisplayConsole("Password did not get reset." , Functions.GUIdisplay.testName, 'se')
          elif Test_PasswordRecovery.is_element_present(self, By.XPATH, "//div[@class ='hidden-print']/div[@id='alertMsgContainer']/div[@class='alert alert-error']"):
            driver.quit()
            automatedSmokeTest.GUIFunctions.outputDisplayConsole("The 'Reset Password' token is expired.", Functions.GUIdisplay.testName, 'se')
          else:
            driver.quit()
            automatedSmokeTest.GUIFunctions.outputDisplayConsole("'Reset Password' button from email is not working correctly." , Functions.GUIdisplay.testName, 'se')
