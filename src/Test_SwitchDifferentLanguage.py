...
class Test_SwitchDifferentLanguage(unittest.TestCase):
    ...
    def test_switch_different_language(self):
        ...
        if check != "Assessment einrichten/ Bericht anfordern":
          automatedSmokeTest.GUIFunctions.outputDisplayConsole("Deutsch - Deutschland language is different than its original language" , testName,'se')
        else: 
          ...
          if check != "Order a Report/Assessment":
            automatedSmokeTest.GUIFunctions.outputDisplayConsole("English - UK language is different than its original language" , testName,'se')
          else:
            # click | id=localeSelector |
            driver.find_element_by_id("localeSelector").click()
            # click | link=日本語 - 日本 |
            ...
            if check != "受検登録とレポートオーダー":
              automatedSmokeTest.GUIFunctions.outputDisplayConsole("日本語 - 日本 language is different than its original language" , testName,'se')
            else:
              # click | id=localeSelector |
              driver.find_element_by_id("localeSelector").click()
              # click | link=한국어 - 한국 |
              ...
              check = driver.find_element_by_id("dashboardOrderReport").text
              if check != "보고서/평가 주문":
               automatedSmokeTest.GUIFunctions.outputDisplayConsole("한국어 - 한국 language is different than its original language" , testName,'se')
              else:
                # click | id=localeSelector |
                driver.find_element_by_id("localeSelector").click()
                ...
                if check != "English - US":
                  automatedSmokeTest.GUIFunctions.outputDisplayConsole("English - US language is different than its original language" , testName,'se')
                else: 
                  automatedSmokeTest.GUIFunctions.outputDisplayConsole("Webpage changes its language according to its locale" , testName,'s')
