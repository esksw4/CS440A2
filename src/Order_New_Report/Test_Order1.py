	...
	def test_order_Exist_Job_Title(self):
		...
		if checkAlsoNotify == "display: block;":
			...
			if not Test_Order1.is_element_present(self, By.XPATH, "//div[@id='deliverToDiv']/div[1]/div[@class='token']"):
				driver.quit()
				automatedApplicaitonGUI.GUIFunctions.outputDisplayConsole("Please enter valid Name for 'Also Notify'.", 'e')
			else:
			...
			driver.quit()
			automatedApplicaitonGUI.GUIFunctions.outputDisplayConsole("The job title, you entered is not available; therefore, 'Also Notify' is not available. Please enter correct existing job title.", 'e')
		...
		if (checkErrorMsg == True):
			...
			automatedApplicaitonGUI.GUIFunctions.outputDisplayConsole("Please enter valid additional information. Cannot process with given input.", 'e')
			# print(GUItkinter.py)
				...
				if whichRow != 9999:
					...
					if "Register" == registerCheck and Functions.CustomInfo['Email Address'] == emailCheck:
						...
						if checkConfirmationMessage == True:
							...
							driver.close()
							automatedApplicaitonGUI.GUIFunctions.outputDisplayConsole("Cancelled Succesfully" , 's')
