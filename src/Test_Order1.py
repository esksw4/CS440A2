...
class Test_Order1(unittest.TestCase):
	...
	def test_order_Exist_Job_Title(self):
		...
		# checkNumError = 0
		Functions.GUIdisplay.testName = "Order Existing Title with New Assessee"
		...
		if checkAlsoNotify == "display: block;":
			...
			if not Test_Order1.is_element_present(self, By.XPATH, "//div[@id='deliverToDiv']/div[1]/div[@class='token']"):
				driver.quit()
				automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: Please enter valid Name for 'Also Notify'.", Functions.GUIdisplay.testName, 'ie')
			else:
			...
			driver.quit()
			automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: The job title, you entered is not available; therefore, 'Also Notify' is not available. Please enter correct existing job title.", Functions.GUIdisplay.testName, 'ie')
		...
		if (checkErrorMsg == True):
			...
			automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: Please enter valid additional information. Cannot process with given input.", Functions.GUIdisplay.testName,  'ie')
			# print(GUItkinter.py)
		...
		try: 
			...
			automatedSmokeTest.GUIFunctions.outputDisplayConsole("Proctored checkbox cannot be pressed automatically. Please manually test the proctored checkbox", Functions.GUIdisplay.testName, 'm')
		...
		try:
			...
			if Test_Order1.is_element_present(self, By.CLASS_NAME, "alert.alert-error.alert-dismissable"):
				driver.quit()
				automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: Please choose different Tag Name. That Tag name exists in the system.", Functions.GUIdisplay.testName, 'ie')
		except:
			try:
				time1.sleep(2)
				...
				automatedSmokeTest.GUIFunctions.outputDisplayConsole("Tag is not created. Please check 'Creat New Tag' functionality.",Functions.GUIdisplay.testName, 'se')
		...
		if checkErrorMsg == True:
			driver.quit()
			automatedSmokeTest.GUIFunctions.outputDisplayConsole("Input Error: Please choose different email for assessee. That email address already exists in the system.", Functions.GUIdisplay.testName, 'ie')
				...
				if whichRow != 9999:
					...
					if "Register" == registerCheck and Functions.CustomInfo['Email Address'] == emailCheck:
						...
						if checkConfirmationMessage == True:
							...
							driver.close()
							automatedSmokeTest.GUIFunctions.outputDisplayConsole("%s tested succesfully" %Functions.GUIdisplay.testName, Functions.GUIdisplay.testName , 's')

						else:
							# print("7")
							# print("Cancellation Failed")
							automatedSmokeTest.GUIFunctions.outputDisplayConsole("Cancellation Failed", Functions.GUIdisplay.testName , 'se')
					else:
						# print("8")
						...
						automatedSmokeTest.GUIFunctions.outputDisplayConsole("Register page email address is not same as user input email address", Functions.GUIdisplay.testName, 'se')
						# print("Email address is not same as user input email address")
				else:
					# print("9")
					automatedSmokeTest.GUIFunctions.outputDisplayConsole("The name is not found from the report list", Functions.GUIdisplay.testName, 'se')
					# print("Name does not match")

			else:
				# print("10")
				# print("The page is not directed to 'View Reports' page")
				automatedSmokeTest.GUIFunctions.outputDisplayConsole("The page is not directed to 'View Reports' page", Functions.GUIdisplay.testName, 'se')
